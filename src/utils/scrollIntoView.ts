 ...
 */
function easeout(dom, destination, rate, callback) {
...
/**
 * 滚动到指定元素
 * @param {Object} el 当前dom元素 或者 css选择器
 * @param {DOM} wrapper 要滚动的元素（一般为window或者父元素）
 * @param {Number} offset 元素距离顶部的偏移量
 */
export function scrollIntoView(el: HTMLElement | string, wrapper = window, offset = 200) {
  if (!el) return;
  if (typeof offset !== 'number') return;
  let dom: HTMLElement = el as any;
  if (typeof el === 'string') {
