...
const projectList = reactive([
  ...
  { id: 2, name: $f('Flare'), style: '--r: 2;--c: 1;--s:3;', time: '2023', image: getImage('home/project/flare.png'), link: 'https://flat-cake-9560.on.fleek.co' },
  { id: 3, name: $f('BlockPulse'), style: '--r: 3;--c: 4;--s: 4;', time: '2022', image: getImage('home/project/profile.png'), link: '/' },
])
    ...
    <div class="grid">
      <figure class="grid__item" v-for="item in projectList" :key="item.id" :style="item.style">
        ...
        <figcaption class="grid__item-caption">
          <h3>{{ $t(item.name) }}</h3> <span>{{ item.time }}</span>
        </figcaption>
      </figure>
    </div>
  </section>
